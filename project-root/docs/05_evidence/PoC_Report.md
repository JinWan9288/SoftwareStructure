...
### 验证的架构假设
验证 ADR-001a 提出的假设：通过 MQ 异步解耦订单与权益服务，能够在第三方支付平台发生高并发重复投递时，实现有效削峰，并保证权益数据的最终一致性（不漏发、不多发）。
...
- **验证标准**: 断路器在 10 秒内打开（OPEN），后续请求立即触发快速失败（Fail-fast），网关层返回“正在排队中”降级提示，且 Live Access Service 的内存和线程池指标保持稳定。
...
- **后续行动**: 将断路器半开 (HALF_OPEN) 状态的探测请求数调整为 10，以平滑恢复流量。
